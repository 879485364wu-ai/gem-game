...
export function createOnlineDispatcher(env,{folder=root,now=Date.now}={}) {
  ...
  return async function dispatch(req){
    ...
    if(url.pathname==='/healthz' && req.method==='GET')return json({ok:true,release:'causality-20260907'});
    ...
    if(url.pathname==='/api/qiluo/status' && req.method==='GET')return json({configured:true,mode:'live',release:'causality-20260907',maxModelTurns:30,voices:voiceStatus(env)});
