...
export function createOnlineDispatcher(env,{folder=root,now=Date.now}={}) {
  ...
  return async function dispatch(req){
    ...
    if(url.pathname==='/healthz' && req.method==='GET')return json({ok:true,release:'chapter-one-20260907.3'});
    ...
    if(url.pathname==='/api/qiluo/status' && req.method==='GET')return json({configured:true,mode:'live',release:'chapter-one-20260907.3',maxModelTurns:30,voices:voiceStatus(env)});
...
if(process.argv[1] && import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){
  try{const port=Number(process.env.PORT||10000);const server=createNodeServer(process.env);server.requestTimeout=120000;server.on('error',()=>{console.error('服务启动失败，请检查部署设置。');process.exitCode=1});server.listen(port,'0.0.0.0',()=>console.log('WeFans 在线体验已启动，端口 '+port));}
