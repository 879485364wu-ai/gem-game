...
import { unseal, unsealChapter } from './story-api.mjs';
import { spokenDialogue } from './dialogue.mjs';

export function voiceStatus(env) {
  const paused=env.VOICE_PAUSED==='true';
  return { chen: !paused&&Boolean(env.MINIMAX_API_KEY && env.CHEN_VOICE_ID), zhao: !paused&&Boolean(env.MINIMAX_API_KEY && env.ZHAO_VOICE_ID), provider:'MiniMax', samples:!paused, paused };
}
const previews={
...
export function createVoiceHandler(env) {
  ...
  async function synthesize(voiceId,dialogue) {
    if(env.VOICE_PAUSED==='true')return Response.json({error:'角色语音已暂停。',paused:true},{status:503});
  ...
  const handler=async function voice(request) {
    ...
    if(!body||typeof body.token!=='string' || body.token.length>500000 || typeof body.messageId!=='string' || body.messageId.length>100 || (body.story!==undefined&&body.story!=='chapter-one') || Object.keys(body).some(k=>!['token','messageId','story'].includes(k))) return Response.json({error:'语音请求无效。'},{status:400});
    let state;
    try {state=await (body.story==='chapter-one'?unsealChapter:unseal)(body.token,env.QILUO_STATE_SECRET)} catch{return Response.json({error:'请重新进入体验。'},{status:400})}
